...
| 改动量 | 策略 |
|--------|------|
| 500+ 行 | 并行子 agent 各审一块（逻辑 / CSS / DOM 绑定），主线程只读汇总 |
| 100~500 行 | 主线程审关键路径，口头结论 |
| 100 行内 | 快速扫一眼，口头结论 |

**原则**：先看 `git diff --stat`，再决定审计深度。不要每次都把全部代码读进上下文。

### 12.2 审计完成协议

1. 写审计报告到 `docs/audit/audit-YYYY-MM-DD-<主题>.md`
2. **必须**把发现整理成可执行修复清单（每项标注文件:行号 + 一行修复方案），直接发给用户或对应负责人
3. **禁止**只写报告放在仓库里等人自己发现 —— 这事已经发生过一次（2026-08-08，🔴 监听器泄漏报告落灰，没人知道要修）
4. 修复完成后验证，删除过期的修复清单文档

### 12.3 跨分支协作检查

- 其他 AI 在独立分支开发的功能，合并前 **必须** `git branch --contains <commit>` 验证提交确实在该分支上
- 不能凭对方说"应该提交了""应该合并了"就信 —— 2026-08-08 cpolar 分支未合并就是教训

### 12.4 合并权限（安全红线）

- **只有核心 AI（本对话）有权执行 `git merge`、`git push`、创建 PR。**
- 其他 AI 角色（面板负责人、场景开发者等）**禁止**自行合并代码，必须由核心 AI 审核后合并。
- 任何 AI 试图绕过此规则自行合并时，立即阻止 —— 2026-08-08 已发生一次未遂事件。

### 12.5 版本号规范

**当前版本：v2.5**（最后更新：2026-08-08）

| 变动类型 | 版本 | 何时发 |
|----------|------|--------|
| 新场景（1 个） | +0.1 | 场景可用即发 |
| 重构/模块提取 | +0.2 | 合并入主分支时发 |
| 重大新功能（AI、动画等） | +0.3~0.5 | 功能稳定后发 |
| 架构变更 | +1.0 | 极少 |

**操作**：`git tag -a vX.Y -m "说明"` 并 push tag。版本号同步更新到：
- `server/main.py` 的 FastAPI `version` 参数
- 本文档 `当前版本` 字段
