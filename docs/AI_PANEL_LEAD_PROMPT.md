...
| `la_grid_settings` | `{range}` 3D 网格可视范围（半轴单位数，默认5=±5，每格=1单位） | `settings-menu.js` `initSettingsMenu()` |
| `la_color_theme` | `{accent, bgPrimary, bgSecondary, bgNav, green, red}` 颜色覆盖 | `settings-menu.js` `initSettingsMenu()` |
| `la_param_ranges` | `{sceneId: {paramKey: {min, max}}}` 参数自定义范围 | `settings-menu.js` 设置菜单 + `scene-base.js` `_buildParams()` |
| `la_settings_collapsed` | `{group: bool}` 设置菜单各分组折叠状态 | `settings-menu.js` `initSettingsMenu()` |
